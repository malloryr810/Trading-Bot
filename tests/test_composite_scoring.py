...
class TestNoSupportedCategories:
    ...
    def test_only_risk_signals_raises(self):
        with pytest.raises(ScoringError, match="No supported signal categories"):
            score_signals("AAPL", [_risk("R1")])

    def test_news_and_risk_only_raises(self):
        with pytest.raises(ScoringError, match="No supported signal categories"):
            score_signals("AAPL", [_news("N1"), _risk("R1")])
...
class TestRenormalisation:
    def test_tech_only_weight_is_100pct(self):
        # composite should equal technical_score exactly (weight = 0.6/0.6 = 1.0)
        impact = 0.3
    ...
    def test_fund_only_weight_is_100pct(self):
        # composite should equal fundamental_score exactly (weight = 0.4/0.4 = 1.0)
        impact = 0.6
...
class TestUnsupportedCategoriesIgnored:
    ...
    def test_risk_ignored_alongside_fundamental(self):
        fund = [_fund("F1", 0.3)]
        risk = [_risk("R1", 0.9)]
        r_with_risk = score_signals("AAPL", fund + risk)
        r_fund_only = score_signals("AAPL", fund)
        assert r_with_risk.score == pytest.approx(r_fund_only.score)

    def test_news_and_risk_ignored_alongside_both_supported(self):
        tech = [_tech("T1", 0.4)]
        fund = [_fund("F1", 0.0)]
        unsupported = [_news("N1", 0.9), _risk("R1", 0.9)]
        r = score_signals("AAPL", tech + fund + unsupported)
        r_clean = score_signals("AAPL", tech + fund)
        assert r.score == pytest.approx(r_clean.score)

    def test_unsupported_signals_not_in_sub_scores(self):
        # NEWS/RISK signals should not inflate the score
        tech = [_tech("T1", 0.0)]
        news = [_news("N1", 1.0)]
